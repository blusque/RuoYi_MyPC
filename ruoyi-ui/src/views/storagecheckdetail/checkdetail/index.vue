<template>
  <div class="app-container">
    ...
    <el-row :gutter="10" class="mb8">
      <el-col :span="1.5">
        <el-button
          ...
          size="mini"
          @click="handleExport"
          v-hasPermi="['storagecheckdetail:checkdetail:export']"
        >导出</el-button>
      </el-col>
    ...
    <el-table v-loading="loading" :data="checkdetailList" @selection-change="handleSelectionChange">
      ...
      <el-table-column label="盈亏数量" align="center" prop="number" />
    </el-table>
    ...
    <el-dialog :title="title" :visible.sync="open" width="500px" append-to-body>
      <el-form ref="form" :model="form" :rules="rules" label-width="80px">
        <el-form-item label="盘点单编号" prop="checkId">
          <el-input v-model="form.checkId" placeholder="请输入盘点单编号" />
        </el-form-item>
        <el-form-item label="物料编号" prop="materialId">
        ...
        <el-form-item label="盈亏数量" prop="number">
          <el-input v-model="form.number" placeholder="请输入盈亏数量" />
        </el-form-item>
      </el-form>
...
<script>
import { listCheckdetail, getCheckdetail, delCheckdetail, addCheckdetail, updateCheckdetail } from "@/api/storagecheckdetail/checkdetail";
...
export default {
  ...
  methods: {
    ...
    getList() {
      this.loading = true;
      listCheckdetail(this.queryParams).then(response => {
        this.checkdetailList = response.rows;
      ...
      this.$refs["form"].validate(valid => {
        if (valid) {
          if (this.form.id != null) {
            updateCheckdetail(this.form).then(response => {
              this.$modal.msgSuccess("修改成功");
              this.open = false;
              this.getList();
            });
          } else {
            addCheckdetail(this.form).then(response => {
    ...
    /** 导出按钮操作 */
    handleExport() {
      this.download('storagecheckdetail/checkdetail/export', {
        ...this.queryParams
      }, `checkdetail_${new Date().getTime()}.xlsx`)
    }
  }
};
</script>
